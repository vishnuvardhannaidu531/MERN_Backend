...
app.post("/cart/:id" , (req , res)=>{
    ...
    if(index ==-1){
        ...
        const item = AddtoCart.find(info => info?.Item?.id === id);
        // console.log(item)
        //If that item not presnt already in the cart then we needs to add that item into the cart so 
        if(!item){
            const Item = FoodMenu.find(info => info.id === id);
            AddtoCart.push({Item:Item,total_items:1});
        }else{
            item.total_items++;
        }
...
app.delete("/cart/:id" , (req,  res)=>{
    const id = parseInt(req.params.id);
    const index = AddtoCart.findIndex(info => info.Item.id === id ) ;
    if(index == -1){
        res.status(404).send("Item not present in  the Cart") ;
    }else{
        const InCart = --AddtoCart[index].total_items;
        if(InCart == 0){
...
app.use("/dummy" , (req ,res) => {
    try{
        // JSON.parse({id:10,name:"vishnu"})
        // JSON.parse("Invalid json"); 
        // JSON.parse('{"id":10,"name":"vishnu"}')
        throw new Error("BROKEN")
        res.send("Data Converted Successfully")
    }catch(err){
        res.send("Some error has occured    "+err)
    }
